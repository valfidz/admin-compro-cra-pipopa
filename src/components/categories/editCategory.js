...
const EditCategory = (props) => {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
        try {
            const response = await axios.get(`${be_url}/api/categories/${categoryId}`, {
                headers: {
            ...
            setErrorMessage("Error loading category data");
            setShowError(true);
        }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${be_url}/api/categories/${categoryId}`, data, {
        headers: {
          "Authorization": `Bearer ${authToken}`,
