...
export const PostsTable = () => {
    ...
    //         try {
    //             await axios.delete(`${be_site}/api/posts/${postId}`, {
    //                 headers: {'Authorization': `Bearer ${authToken}`},
    //             });
    //             setTableItems(tableItems.filter(item => item.id !== postId));
    //             setShowNotif(true);
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td className="py-4 whitespace-nowrap">
                                        <div className="flex items-center justify-end gap-4">
                                            ...
                                            {/* This button is for delete */}
                                            <button aria-label="Delete post" title="Delete-post" className="text-gray-600 hover:text-red-600 transition-colors duration-200" onClick={() => handleDelete(item.id)}>
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
